...
void draw_features( cv::Mat &output, struct feature* feat, int n );
...
//void draw_oxfd_feature( IplImage*, struct feature*, cv::Scalar );

int import_lowe_features( char*, struct feature** );
int export_lowe_features( char*, struct feature*, int );
void draw_lowe_features( cv::Mat &output, struct feature*, int );
void draw_lowe_feature( cv::Mat &output, struct feature*, cv::Scalar );
...
void draw_features( cv::Mat &output, struct feature* feat, int n )
{
  ...
  switch( type )
    ...
    case FEATURE_OXFD:
      ...
      break;
    case FEATURE_LOWE:
      draw_lowe_features( output, feat, n );
      break;
    default:
...
void draw_oxfd_feature( IplImage* img, struct feature* feat, cv::Scalar color )
{
  ...
  CvEllipse( img, cv::Point( feat->x, feat->y ), cv::Size( l2, l1 ), alpha,
	     0, 360, cv::Scalar(0,0,0), 3, 8, 0 );
  CvEllipse( img, cv::Point( feat->x, feat->y ), cv::Size( l2, l1 ), alpha,
	     0, 360, color, 1, 8, 0 );
  CvLine( img, cv::Point( feat->x+2, feat->y ), cv::Point( feat->x-2, feat->y ),
	  color, 1, 8, 0 );
  cv::line( img, cv::Point( feat->x, feat->y+2 ), cv::Point( feat->x, feat->y-2 ),
	  color, 1, 8, 0 );
}
*/
...
/*
  ...
  @param n number of features
*/
void draw_lowe_features( cv::Mat &output, struct feature* feat, int n )
{
  cv::Scalar color = cv::Scalar( 255, 255, 255 );
  int i;
  ...
  for( i = 0; i < n; i++ )
    draw_lowe_feature( output, feat + i, color );
}
...
/*
  ...
  @param color color in which to draw
*/
void draw_lowe_feature( cv::Mat &output, struct feature* feat, cv::Scalar  color )
{
  ...
  cv::line( output, start, end, color, 1, 8, 0 );
  cv::line( output, end, h1, color, 1, 8, 0 );
  cv::line( output, end, h2, color, 1, 8, 0 );
}
